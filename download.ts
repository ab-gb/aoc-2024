...
const options = {
	...
	solution_test_filename: "solution-test.ts",
	challenge_filename: "challenge.txt",
	input_filename: "input.txt",
	boilerplate_filename: "boilerplate.ts",
	boilerplate_test_filename: "boilerplate-test.ts",
	year: "2023",
	first_day: 1,
	last_day: 25,
	cookie: "",
...
const setOptionsFromCommand = async (): Promise<string[]> => {
	...
	if(Deno.args[2]?.toLowerCase() === 'js'){
		options.boilerplate_filename = options.boilerplate_filename.replace('.ts', '.js');
		options.solution_filename = options.solution_filename.replace('.ts', '.js');
	}
	if (errors.length === 0) {
...
const getAdvent = async (): Promise<void> => {
	for (let d = options.first_day; d <= options.last_day; d++) {
		...
		const solution_test_path = `${base_path}${options.solution_test_filename}`;
		...
		try {
			...
			const boilerplatetest = await Deno.readTextFile(options.boilerplate_test_filename);
			await Deno.mkdir(base_path);
			await Deno.writeTextFile(solution_path, boilerplate);
			await Deno.writeTextFile(solution_test_path, boilerplatetest);
